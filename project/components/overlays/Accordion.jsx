 ...
 */
export function Accordion({ title, count, open = false, onToggle, children, style = {} }) {
  ...
  return (
    <div style={{
      ...
      >
        <span style={{
          flex: 1, fontFamily: 'var(--font-display)', fontSize: 'var(--text-body-lg)',
        ...
        <span style={{
          fontSize: 16, color: open ? 'var(--accent)' : 'var(--text-secondary)',
      ...
      <div style={{
        height: open ? h : 0, opacity: open ? 1 : 0, overflow: 'hidden',
        transition: 'height var(--dur-slow) var(--ease-out), opacity var(--dur-base) var(--ease-standard)',
      }}>
        <div ref={bodyRef} style={{ paddingBottom: 12 }}>{children}</div>
      </div>
    </div>
  );
}
